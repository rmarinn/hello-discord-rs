...
impl Serialize for CommandOptionChoice {
        ...
        S: serde::Serializer,
    {
        ...
        match &self.value {
            CommandOptionChoiceValue::Int(val) => s.serialize_field("value", val)?,
            CommandOptionChoiceValue::Float(val) => s.serialize_field("value", val)?,
            CommandOptionChoiceValue::String(val) => {
                if val.len() > 100 {
...
pub struct CommandOptionChoiceBuilder {
    name: Option<String>,
    value: Option<CommandOptionChoiceValue>,
}
...
impl CommandOptionChoiceBuilder {
    ...
    pub fn value(mut self, value: CommandOptionChoiceValue) -> Self {
    ...
    pub fn build(self) -> Result<CommandOptionChoice, Box<dyn std::error::Error>> {
        ...
        if let Some(value) = &self.value {
            match value {
                CommandOptionChoiceValue::String(string_val) => {
